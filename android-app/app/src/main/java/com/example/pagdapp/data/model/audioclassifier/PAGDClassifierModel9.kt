...
class PAGDClassifierModel9 @Inject constructor(
    ...
    private val output_tensor =
        Array<Array<FloatArray>>(64) { Array<FloatArray>(129) { floatArrayOf(1.0f) } }
    ...
    init {
        try {
            ...
            extractLabels(arrayListOf("Gun1_model9", "Gun2_model9", "Gun4_model9"))
    ...
    suspend fun thread_work() = withContext(Dispatchers.IO) {
        ...
        while (isActive && isRecording) {
            ...
            Log.e(
                "classifyAudio",
                output2[0][0].toString() + " - PAGD_9" + " delay: "
 ...
 */
    ...
    override fun includeAllCategories() {
        ...
        Log.e("switchClassifier", "included all PAGD_9")
    }

    override fun excludeAllCategories() {
        categoriesToInclude.clear()
        Log.e("switchClassifier", "excluded all PAGD_9")
    }
    ...
    override fun allCategoriesIncluded(): Boolean {
        for (category in thisAIModelsCategories) {
            if (!category.isIncluded) {
                Log.e("switchClassifier", category.title + "- PAGD_9")
                return false
            }
