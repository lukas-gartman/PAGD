...
/**
 * This class implements a simple version of the <i>k-Means Clustering</i>
 ...
 * improvement to the <i>mean quantization error (MQE)</i>.
 * <br>
 *
...
public class KMeansClustering
{
   ...
   * @param normalize boolean true if data-points should be normalized before
   *                          processing, otherwise false;
   * @param maxIterations int maximal number of iterations the algorithm should
   *                          perform
   * @throws IllegalArgumentException raised if method contract is violated
   */
  public KMeansClustering(int numberClusters, PointList pl, boolean normalize, int maxIterations) throws IllegalArgumentException
  {
    ...
    //get mean, variance and standard deviation of the data
   ...
   * till the mean quantization error(MQE) stops decreasing.
   */
  public void run()
  {
    double MQE = 0.99d * Double.MAX_VALUE; //the mean quantization error of the current clustering
    double oldMQE = Double.MAX_VALUE;      //the mean quantization error of the last clustering
    ...
    {
      //store old mean quantization error
      oldMQE = MQE;
      MQE = 0;
      ...
      for(int k = 0; k < data.size(); k++)
      {
        ...
        //increase mean quantization error
        MQE += minDistance;
  ...
  protected void createFullCovarianceMatrices()
  {
    ...
    for(int k = 0; k < data.size(); k++)
    {
      ...
      //add the diff to the specific matrix
    ...
    for(int i = 0; i < numberClusters; i++)
    {
      ...
      if(covariances[i].rank() < dimension)
      {
        ...
        //that center is responsible for a reasonable number of points.
  ...
  /**
   * Returns the mean of the specified cluster.<br>
   * <br>
   * <b>Note:</b> Before calling this method there should exist a valid
  ...
  /**
   * Returns the weights of all clusters in one array.<br>
   * <br>
   * <b>Note:</b> Before calling this method there should exist a valid
   * clustering, which one can create by calling the <code>run()</code> method.
   *
