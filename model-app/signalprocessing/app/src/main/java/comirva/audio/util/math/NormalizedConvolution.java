 ...
 * For extracting the vector out of the matrix one can specify, if column or
 * row vectors should be used. This is done for all vectors in the matrix. <br>
 * <br>
...
public class NormalizedConvolution
{
   ...
   *
   * @param filter double[] the filter used for the convolution e.g. a gaussian
   *                        filter
   *
   * @throws IllegalArgumentException the given filter must not be a null value
   *                                  and must have an odd length greater or
   *                                  equal to 3
   */
   ...
   *
   * @param A Matrix the matrix to convolute with
   * @param firstDimension boolean if true each of the row vectors will get
   *                               convoluted with the filter, otherwise each
   *                               of the column vectors will be used to
   *                               convolute with
   * @return Matrix as result the completely convoluted matrix will be returned
   */
   ...
   *
   * @param data double[] the input vector to convolute with the filter
   * @param filterArray double[][] a filter array containing the normalized
   *                               filter and truncated normalized filters for
   *                               the start and ending elements of the vector.
   ...
   *
   * @param filter double[] filter to compute normalized and truncated versions of
   * @return double[][] an array containing normalized and truncated version of
   *                    the given filter
   */
   ...
   *
   * @param filter double[] the filter to normalize
   */
